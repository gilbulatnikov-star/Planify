...
export function SearchableSelect({
  ...
  useEffect(() => {
    if (isOpen) {
      setSearch("");
      setHighlightIndex(0);
      // Focus the search input after a tick so the dropdown is rendered
  ...
  return (
    <div ref={containerRef} className={cn("relative", className)} onKeyDown={handleKeyDown}>
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute z-50 mt-1 w-full min-w-[200px] rounded-lg bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10 overflow-hidden animate-in fade-in-0 zoom-in-95 slide-in-from-top-2 duration-100">
          {/* Search input */}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((option, i) => (
                <button
                  ...
                  className={cn(
                    "relative flex w-full items-center gap-1.5 rounded-md py-1.5 pr-2 pl-7 text-sm outline-none select-none transition-colors",
